...
static ROWS: &[Row] = &[
    ...
    Row::Act(Action::Refresh, "refresh prices and visible data"),
    Row::Act(Action::ToggleBare, "bare mode: hide the header and footer"),
    Row::Act(Action::NextTheme, "next color preset (Save keeps it)"),
    Row::Act(Action::PrevTheme, "previous color preset"),
    ...
    Row::Act(Action::Position, "set what you hold of the ticker"),
    Row::Act(Action::Quit, "quit"),
    Row::Fixed("esc", "quit, close an overlay"),
    Row::Fixed("ctrl-c", "force quit"),
    Row::Group("Navigate"),
    Row::Act(Action::Up, "move up (ticker, article or event)"),
    Row::Act(Action::Down, "move down; last article row loads more"),
    Row::Act(Action::Left, "previous ticker (News/Insider/Earnings)"),
    Row::Act(Action::Right, "next ticker (News/Insider/Earnings)"),
